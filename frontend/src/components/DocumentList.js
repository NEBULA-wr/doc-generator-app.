...
const DocumentList = ({ documents, onActionStart, onActionEnd, loadingDocId, onDelete, onError }) => {
  ...
  const API_URL = process.env.REACT_APP_API_URL;
   axios.post(`${API_URL}/generate-pdf`, 
      ...
      // En lugar de alert(), llamamos a la función onError
      onError(error.message || t('error_generic'));
    } finally {
      onActionEnd();
    }
  };
                ...
                <td>
                  <div className={styles.clientCell}>
                    ...
                    <div className={styles.mobileDetails}>
                      <span>{doc.form_data.invoiceNumber}</span>
                      <span>{new Date(doc.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                </td>
                <td className={styles.hideOnMobile}>{doc.form_data.invoiceNumber}</td>
                <td className={styles.hideOnMobile}>{new Date(doc.created_at).toLocaleDateString()}</td>
                <td>
                  <div className={styles.actions}>
                    <button className={styles.actionButton} title="Ver" onClick={() => handleDocumentAction(doc, true)} disabled={loadingDocId === doc.id}>
                      {loadingDocId === doc.id ? '...' : <EyeIcon />}
                    </button>
                    <button className={styles.actionButton} title="Descargar" onClick={() => handleDocumentAction(doc, false)} disabled={loadingDocId === doc.id}>
                      {loadingDocId === doc.id ? '...' : <DownloadIcon />}
                    </button>
                    <button className={`${styles.actionButton} ${styles.deleteButton}`} title="Borrar" onClick={() => onDelete(doc.id)} disabled={loadingDocId === doc.id}>
                      <TrashIcon />
                    </button>
                  </div>
                </td>
